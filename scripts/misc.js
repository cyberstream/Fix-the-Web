...
var CONFIG = {
    ...
    // Twitter OAuth configuration; do not modify.
    twitter: {
...
function getUserName(callback) {
    ...
    if ( CONFIG.offline ) {
        if ( typeof callback == 'function' ) callback ({text: '{"screen_name": "FTW_test"}'});
        return true;
    }
    else if (isLoggedIn() && typeof OAuth !== 'undefined') {
        if (typeof callback !== 'function') {
            callback = function(data) {
                ...
                console.log('[FtW OAuth message] ' + data);
            };
        }
...
function updateReportSummary(callbackFunc) {
    ...
    sendRequest("get", updateReportUrl, function(data) {
        ...
        if (typeof ToolbarIcon == 'object' && "updateBadge" in ToolbarIcon)
...
if (typeof widget.preferences.lastSummaryUpdate == 'undefined')
    updateSummaryList();
...
if (widget.preferences.getItem('update-interval')) {
    ...
    setInterval(function() {
        ...
        if (typeof lastUpdateTime != 'undefined') {
            if ((currentTime - lastUpdateTime) >= (1000 * 60 * 60 * 8))
                updateSummaryList();
        }
