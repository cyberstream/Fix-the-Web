opera.extension.onmessage = function(event) {    
    ...
    if (event.data == 'reply') {
        ...
        if (!document.getElementById('fix-the-web-comment-frame')) {
            ...
            + '#resize-frame.hidden {opacity:0} #resize-frame.display {opacity:1} #resize-frame:before { position:absolute; left:46%; content:""; width:100px; height:100%; background: radial-gradient(ellipse 3px 3px at 25% 50%, #555 100%, transparent), radial-gradient(ellipse 3px 3px at 36% 50%, #555 100%, transparent), radial-gradient(ellipse 3px 3px at 47% 50%, #555 100%, transparent), radial-gradient(ellipse 3px 3px at 25% 54%, #bbb 100%, transparent), radial-gradient(ellipse 3px 3px at 36% 54%, #bbb 100%, transparent), radial-gradient(ellipse 3px 3px at 47% 54%, #bbb 100%, transparent); }'
